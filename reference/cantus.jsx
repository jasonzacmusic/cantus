...
// ════════════════════════════════════════════════════════════
//  NSM LOGO (base64, transparent background — processed)
// ════════════════════════════════════════════════════════════
// Placeholder — real base64 injected below this file after create.
const NSM_LOGO = null; // Logo removed — Claude Code will supply the final asset at /public/nsm-logo.png
...
export default function Cantus() {
  ...
  const initAudio = useCallback(async () => {
    ...
    initPromiseRef.current = (async () => {
      try {
        ...
        // Main voice: triangle, bright, pianistic attack
        const main = new Tone.PolySynth(Tone.Synth).toDestination();
        main.set({
          oscillator: { type: 'triangle' },
          envelope: { attack: 0.015, decay: 0.32, sustain: 0.48, release: 1.6 },
          volume: -5,
        });

        // Pad voice: sine, warm body an octave below — gives the chord weight
        ...
        // Metronome click: crisp sine tick with very short envelope
  ...
  // CLASSIC: a bank of progressions that have powered popular music for
  // centuries. Each tap of the button cycles to a different one (and a
  // random key), so the user can keep tapping to find the one they love.
  //
  // Each pattern is an array of [degree_in_semitones_from_tonic, quality]
  ...
  const CLASSIC_KEYS = [60, 62, 64, 65, 67, 69, 71]; // C D E F G A B
  ...
  const surpriseCinematic = () => {
    ...
    // Roots span a wider set of starting keys — cinematic loves dark keys
    const roots = [57, 58, 59, 60, 61, 62, 63, 64, 65]; // A through F
    const tonic = roots[Math.floor(Math.random() * roots.length)];

    // Voicings: mix root- and 3rd-based quartals across the four chords.
    // Vary per run for textural variety.
    const voicingPlans = [
    ...
    const vPlan = voicingPlans[Math.floor(Math.random() * voicingPlans.length)];
